 ...
 * 3. F5
  */
let connection = node_1.createConnection(new node_1.IPCMessageReader(process), new node_1.IPCMessageWriter(process));
    ...
    if (!scenarioFound) {
        ...
        tableCache.forEach(table => {
            items.push({
                ...
                kind: table.is_table ? node_1.CompletionItemKind.Class : node_1.CompletionItemKind.Interface,
                insertText: table.schemaname == "public" ? table.tablename : table.schemaname + "." + table.tablename
            });
            table.columns.forEach(field => {
                let foundItem = items.find(i => i.label === field.attname && i.kind === node_1.CompletionItemKind.Field && i.detail === field.data_type);
                if (foundItem) {
